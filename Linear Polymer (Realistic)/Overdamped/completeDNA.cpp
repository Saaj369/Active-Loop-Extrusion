...
typedef int NList_t[NUM_PARTICLES * NUM_PARTICLES];                     // Raw
typedef int NLindex2d_t[NUM_PARTICLES][2];                              // Raw
...
void neighbour_list(NList_t &NL, NLindex2d_t &NLsize, c2D_t &POS)
{
    ...
    for (int i = 0; i < NUM_PARTICLES - 1; i++)
    {
        ...
        for (int j = i + 2; j < NUM_PARTICLES; j++)
        {
            vec_t rij = {};
            double dist_sq = 0;
            for (int k = 0; k < 3; k++)
...
{
    bool found = false; // Shared flag for early exit

    #pragma omp parallel
    {
        bool local_found = false; // Local flag for each thread

        #pragma omp for
        for (int i = 0; i < NUM_PARTICLES; i++) {
            if (local_found) {
                continue; // Skip further iterations if flag is already set
            }
            for (int j = 0; j < 3; j++) {
                double Disp = std::abs(POS[i][j] - POS0[i][j]);
                if (Disp > CSKIN) {
                    local_found = true; // Set local flag when condition is met
                    break; // Exit inner loop as we found a large displacement
                }
            }
            if (local_found) {
                #pragma omp critical
                found = true; // Update shared flag in critical section
            }
        }
    }
...
void eval_force_using_NL(c2D_t &NF, c2D_t &POS, NList_t &NL, NLindex2d_t &NLsize)
{
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < NUM_PARTICLES - 1; i++)
        {
            for (int index = NLsize[i][0]; index < NLsize[i][1]; index++)
            {
                ...
                // Compute displacement and squared distance
                #pragma omp simd reduction(+:r2)
                for (int m = 0; m < 3; m++)
                {
                    rij[m] = POS[i][m] - POS[j][m];
                    r2 += rij[m] * rij[m];
                }

                if (r2 <= CUTTOFF_RADIUS2)
                {
                    array<double, 3> Flj = {};
                    // double factor2 = 24 * (2 * pow(r2, -7) - pow(r2, -4));
                    double factor2 = 0;
...
/*******************************************************Integration Step****************************************** */
void underdamped_velocity_verlet_BD_using_NL(g2D_t &GRAN, c2D_t &POS, c2D_t &VEL, c2D_t &F, NList_t &NL, NLindex2d_t &NLsize, c2D_t &POS0, activeSMCs_t &activeSMCs)
{
...
void integrate_and_simulate(float simDuration, float esimDuration, float freq){
    ...
    string POSfile = "dataDNA/POLYLJOFF_5FEB_full.xyz";
    // string stateFile = "dataDNA/polymer_state_7DEC3.txt";
    string CTCFfile = "CTCF_coarse_positions.csv";
    string EPOSfile = "dataDNA/EPOLYLJOFF_5FEB_full.xyz";
    string DEPOSfile = "dataDNA/EPOLYLJOFF_5FEB_full.dump";
    string DPOSfile = "dataDNA/POLYLJOFF_5FEB_full.dump";
    ...
    pcg32 BD_GS{BD_RAND_SEED};             // brownian dynamics verlet
    ...
    pcg32 BD_G{};                          // brownian dynamics verlet
    ...
    neighbour_list(NL, NLsize, POS);                            // Generate NL
    eval_force_using_NL(F, POS, NL, NLsize);                    // Evaluate forces
    copy_POS_data(POS, POS0);                                   // Save positions for NL


    ofstream Eout(EPOSfile);
    if (!Eout) {
        cerr << "Error: Could not open EPOSfile for writing." << endl;
        exit(1);
    }
    // ofstream DEout(DEPOSfile, std::ios::binary);
    // if (!DEout.is_open()) {
    //     std::cerr << "Error: Could not open the DEfile for writing." << std::endl;
    //     exit(1);
    // }
    ...
    for(int i = 0; i < equiSteps; i++){
        ...
        underdamped_velocity_verlet_BD_using_NL(GRAN, POS, VEL, F, NL, NLsize, POS0, activeSMCs);
    ...
    // ofstream Dout(DPOSfile, std::ios::binary);
    // if (!Dout.is_open()) {
    //     std::cerr << "Error: Could not open the DEfile for writing." << std::endl;
    //     exit(1);
    // }
...
int main(){
    float simDuration = 100000;       // 25000 Full simulation time (2e7)
    float esimDuration = 5000;      // Equilibration time (1e6)
    float eMCDuration = 10000;
    float freq = 100;   //10*dtau;                  // Data saving frequency

    integrate_and_simulate(simDuration, esimDuration, freq);
    return 0;
}