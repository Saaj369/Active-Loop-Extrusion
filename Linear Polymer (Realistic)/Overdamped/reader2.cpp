...
void processFrame(std::ifstream& file, int numParticles, int frameNumber, const double cutoff, std::array<std::array<int, 400>, 400> &hicM) {
...
void saveMatrix(const std::array<std::array<int, 400>, 400>& hicMatrix, const std::string& filename) {
...
int main() {
    ...
    std::string fileName = "DNA_21DEC_full.xyz";
    ...
    double cutoff = 1.5;
    std::array<std::array<int, 400>, 400> hicMatrix = {};
    std::string hicFIle = "hicnew21_.txt";
    ...
    // Skip the first half of the frames
    long long framesToSkip = totalFrames / 2;
    for (long long i = 0; i < framesToSkip; ++i) {
        // Skip the number of particles line and the step line
        std::getline(file, line);
        std::getline(file, line);
        // Skip all particle lines
        for (int j = 0; j < numParticles; ++j) {
            std::getline(file, line);
        }
    }

    // Process every 100th frame from the remaining frames
    while (std::getline(file, line)) {
        ...
        // Check if the frame is the 100th frame
        if ((frameNumber - framesToSkip) % 100 == 0) {
